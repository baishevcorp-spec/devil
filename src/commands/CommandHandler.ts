...
`;
  ...
  private countProjectFiles(tree: any): number {
    let count = 0;
    // Служебные папки, которые не являются частью исходного кода проекта
    ...
    function traverse(node: any) {
      // Пропускаем служебные директории и их содержимое
      if (node.type === 'directory' && ignoredDirs.has(node.name)) {
        return;
      }
    ...
    try {
      ...
      try {
        ...
        if (execError.stdout) {
          ...
          logger.info(
            'ESLint нашёл проблемы (exit code ' + execError.code + '), парсим вывод',
            'CommandHandler'
          );
        } else {
